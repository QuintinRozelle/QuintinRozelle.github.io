...
#               Contains Bid class and implementation of interaction
#=======================================================================================
...
BinarySearchTree = NewType('BinarySearchTree', binarySearchTree.BinarySearchTree)
Bid = NewType('Bid', 'Bid')
Node = NewType('Node', 'binarySearchTree.Node')
...
def loadBids(csvPath: str) -> BinarySearchTree:
    """
    Loads bid data from a csv to the binary search tree in memory
    ...
    -------
    BinarySearchTree
        A binary search tree loaded with the data from the csv file
    """
...
def displayLoadMenu(bstRoot : Node) -> int:
    """
    ...
    ----------
    Node
        The root of the BST. Used to check if BST is already laoded with data
    ...
    int
        The user's choice
    """
    if bstRoot is not None:
        choice : str = "0"
        while choice == "0":
            print("A file has already been loaded. What would you like to do?")
            print("  1. Clear binary search tree and load new file")
            print("  2. Add contents of new file to current binary search tree")
...
if __name__ == '__main__':
    bst : binarySearchTree = binarySearchTree.BinarySearchTree()
    choice : int = 0
    while (choice != 9):
        ...
        match choice:
            # Load file
            case 1:
                loadChoice : int = displayLoadMenu(bst.root)
                # load file
